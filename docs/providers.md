...
Authenticated LLMProxy endpoints accept JSON request bodies up to 32 MB by default so image-bearing conversation history can pass without hitting Plug's 8 MB default. Set `LLM_PROXY_BODY_LIMIT_BYTES` to a positive integer to change that bound. The default matches the direct Anthropic Messages API request limit and leaves ample headroom above the observed 8.56 MB Pi session; upstream providers may enforce lower endpoint-specific limits. Authentication and quota checks run before the proxy reads or decodes these larger bodies, and requests beyond the bound receive a structured HTTP 413 response.

LLMProxy configures ReqLLM's Finch transport as eight HTTP/1 pool shards with four connections per shard. This gives each upstream destination bounded capacity for 32 concurrent streams while retaining connection isolation and backpressure; change these application settings only after concurrency and resource validation. Configuration-driven providers receive LLMProxy's provider transport timeout. Cowboy uses that same timeout as its connection idle ceiling and resets the timer whenever it sends response data. LLMProxy also emits SSE comment heartbeats while an upstream enumerable is silent, keeping Cowboy, reverse proxies, and clients alive without changing OpenAI- or Anthropic-compatible event payloads. ReqLLM failures are projected onto canonical status, code, and message fields; internal exception terms, stacktraces, request bodies, and upstream headers are never rendered to clients.
