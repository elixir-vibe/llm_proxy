...
- The bundled Phoenix admin UI is optional and conditionally compiled when Phoenix/LiveView/Volt are available; set `config :llm_proxy, web: false` to skip supervising the endpoint. `LLMProxy.Admin` is the service-owned Incant admin surface; it exposes Ecto schemas convention-first, while modules under `LLMProxy.Admin.Resources.*` override inferred resources automatically. The existing LiveViews remain the parity oracle while Incant contracts and remote admin execution mature.
