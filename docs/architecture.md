...
- The bundled Phoenix admin UI is optional and conditionally compiled when Phoenix/LiveView/Volt are available; set `config :llm_proxy, web: false` to skip supervising the endpoint. `LLMProxy.Admin` is the service-owned Incant admin surface; the existing LiveViews remain the parity oracle while Incant contracts and remote admin execution mature.
