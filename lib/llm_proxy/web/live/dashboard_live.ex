if Code.ensure_loaded?(Phoenix.LiveView) do
  defmodule LLMProxy.Web.DashboardLive do
    ...
    def render(assigns) do
      ...
      <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <.stat_card label="Requests" value={Format.number(@stats.total_requests)} />
          <.stat_card label="Total Spend" value={Format.usd(@stats.total_spend_usd)} />
          <.stat_card label="Input Tokens" value={Format.number(@stats.total_input_tokens)} />
          <.stat_card label="Output Tokens" value={Format.number(@stats.total_output_tokens)} />
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr :for={r <- Enum.take(@stats.recent_usage, 15)} class="border-b border-gray-100">
                    ...
                    <td class="py-1.5 text-right">{Format.number(r.input_tokens)}</td>
                    <td class="py-1.5 text-right">{Format.number(r.output_tokens)}</td>
                    <td class="py-1.5 text-right text-gray-600">{Format.usd(r.cost_usd)}</td>
                    <td class="py-1.5 text-right text-gray-500">{Format.ms(r.duration_ms)}</td>
                    <td class="py-1.5 text-right text-gray-500">{Format.ms(r.ttft_ms)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div :for={s <- @stats.service_stats} class="flex justify-between py-2 border-b border-gray-100">
              ...
              <span class="text-gray-600">{Format.number(s.count)} requests</span>
            </div>
