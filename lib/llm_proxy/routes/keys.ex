defmodule LLMProxy.Routes.Keys do
  ...
  get "/" do
    ...
    keys_with_usage =
      Enum.map(keys, fn k ->
        usage_4h = Storage.get_usage_in_window(k.id, @four_hours_ms)
        usage_week = Storage.get_usage_in_window(k.id, @one_week_ms)
        messages_4h = Storage.get_message_count_in_window(k.id, @four_hours_ms)
        messages_week = Storage.get_message_count_in_window(k.id, @one_week_ms)
        {cache_ratio_4h, _total} = Storage.get_cache_ratio_in_window(k.id, @four_hours_ms)

        %{
          id: k.id,
          name: k.name,
          created_at: k.inserted_at,
          quota_4h_input: k.quota_4h_input,
          quota_4h_output: k.quota_4h_output,
          quota_week_input: k.quota_week_input,
          quota_week_output: k.quota_week_output,
          quota_4h_messages: k.quota_4h_messages,
          quota_week_messages: k.quota_week_messages,
          min_cache_ratio: k.min_cache_ratio,
          allowed_models: k.allowed_models,
          service_quotas: k.service_quotas,
          usage_4h: usage_4h,
          usage_week: usage_week,
          messages_4h: messages_4h,
          messages_week: messages_week,
          ...
          cache_read_tokens: k.cache_read_tokens,
          cache_write_tokens: k.cache_write_tokens
        }
      end)
  ...
  post "/quota" do
    ...
    cond do
      not has_token_quotas and not has_service_quotas ->
