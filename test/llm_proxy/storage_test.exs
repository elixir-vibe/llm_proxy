defmodule LLMProxy.StorageTest do
  ...
  describe "messages, traces, and stats" do
    test "returns exact, searchable message pages with filter options" do
      {:ok, key, _} = Storage.create_key("paged")

      for index <- 1..30 do
        Storage.log_message(%{
          key_id: key.id,
          model: if(rem(index, 2) == 0, do: "model-even", else: "model-odd"),
          route: if(rem(index, 3) == 0, do: "responses", else: "chat"),
          user_message: if(index == 17, do: "unique needle", else: "message #{index}")
        })
      end

      table = %{page: 2, page_size: 10, sort: "-timestamp"}
      page = Storage.page_messages(%{table: table, filters: %{}, search: nil})
      ...
      assert page.options["model"] == ["model-even", "model-odd"]
      assert page.options["route"] == ["chat", "responses"]

      filtered =
        Storage.page_messages(%{
          table: %{page: 1, page_size: 10},
          filters: %{"model" => "model-even"},
          search: nil
        })

      assert filtered.total == 15
      assert Enum.all?(filtered.rows, &(&1.model == "model-even"))
    ...
    test "returns exact, filtered trace pages" do
      {:ok, key, _} = Storage.create_key("traced")
      ...
      for index <- 1..12 do
        assert {:ok, _trace} =
                 Storage.record_trace(%{
                   key_id: key.id,
                   model: if(rem(index, 2) == 0, do: "model-even", else: "model-odd"),
                   provider: if(rem(index, 3) == 0, do: "openrouter", else: "openai"),
                   input_tokens: index,
      ...
      page =
        Storage.page_traces(%{
          table: %{page: 2, page_size: 5, sort: "-input_tokens"},
          filters: %{
            "provider" => "openai",
            "timestamp" => %{
      ...
      assert Enum.all?(page.rows, &(&1.provider == "openai"))
      assert page.options["model"] == ["model-even", "model-odd"]
    end
